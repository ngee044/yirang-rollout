...
namespace Install
{
	namespace
	{
		...
		auto validate_release_id(const std::string& release_id) -> std::expected<void, std::string>
		{
			...
			if (release_id == "." || release_id == ".." || release_id.rfind(kStagingPrefix, 0) == 0)
			{
				return std::unexpected(std::format("release_id must not be a path reference or a staging name: '{}'", release_id));
			}
	...
	auto ReleaseInstaller::install(const std::string& release_id, const std::string& source_directory) -> std::expected<void, std::string>
	{
		...
		if (error)
		{
			...
			return std::unexpected(std::format("cannot publish '{}': {}", target, error.message()));
		}
	...
	auto ReleaseInstaller::installed(void) const -> std::expected<std::vector<std::string>, std::string>
	{
		...
		while (entry != last)
		{
			...
			if (entry->is_directory(error) && name.rfind(kStagingPrefix, 0) != 0)
			{
